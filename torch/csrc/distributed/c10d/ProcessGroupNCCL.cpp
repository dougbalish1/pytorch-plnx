...
ProcessGroupNCCL::WorkNCCL::WorkNCCL(
  ...
  for (uint32_t i = 0; i < devices.size(); ++i) {
    ncclEndEvents_->emplace_back(at::cuda::CUDAEvent(enableTiming ? cudaEventDefault : cudaEventDisableTiming));
  }
...
void ProcessGroupNCCL::registerOnCompletionHook(std::function<void(c10::intrusive_ptr<Work>)>&& hook) {
  TORCH_CHECK(
    enableTiming_,
...
void ProcessGroupNCCL::waitForInFlightWorks() {
...
void ProcessGroupNCCL::workCleanupLoop() {
  ...
  while (!done || !terminateProcessGroup_.load()) {
      ...
      for (auto it = workMetaList_.begin(); it != workMetaList_.end();
        ...
        if (work.isCompleted()) {
          if (onCompletionHook_) {
            std::cout << "==== on completion hook exists\n" << std::flush;
            completedWorkList.splice(completedWorkList.begin(), workMetaList_, it++);
          } else {
    ...
    // Python hook will grab GIL. Release workMetaListMutex_ before acquiring GIL to
    // avoid potential deadlock.

    // TODO @mrshenli:
    // 1. figure out a way to hold python interpreter alive before all hooks are fired
    // 2. we can wait for done, although done can toggle
    // 3. cannot wait for watchdog thread to terminate, because that's only set in dtor.
    try {
      while (!completedWorkList.empty()) {
        std::cout << "=== before process hook\n" << std::flush;
        onCompletionHook_(c10::make_intrusive<WorkNCCL>(completedWorkList.back()));
        std::cout << "=== after process hook\n" << std::flush;
        completedWorkList.pop_back();
      }
    } catch (std::exception& e) {
      std::cout << "===== caught exception!" << std::flush;
      ...
      if (!completedWorkList.empty()) {
        std::cout << "=== completedWorkList is not empty\n" << std::flush;
        completedWorkList.back().setException(std::make_exception_ptr(std::runtime_error(errorStr)));
...
c10::intrusive_ptr<ProcessGroupNCCL::WorkNCCL> ProcessGroupNCCL::initWork(
  ...
  return c10::make_intrusive<ProcessGroupNCCL::WorkNCCL>(
      devices, rank, opType, seq_, profilingTitle, inputs, desyncDebug_, enableTiming_);
}
...
float ProcessGroupNCCL::WorkNCCL::getDuration() const {
  std::cout << "===== in getDuration === " << c10::str(ncclStartEvents_->size()) << std::endl << std::flush;
  TORCH_CHECK(ncclStartEvents_->size() == 1, "getDuration only works for single device per ProcessGroup.");
  TORCH_CHECK(ncclEndEvents_->size() == 1, "getDuration only works for single device per ProcessGroup.");
  TORCH_CHECK((*ncclEndEvents_)[0].query(), "getDuration can only be called after work is succeeded.")
  std::cout << "===== in getDuration passed checks === " << std::endl << std::flush;
